...
/**
 * The class that manages the modules, the command parser consults this if it
 * cannot find the specified command
 *
 * @author Coolway99
 */
public class ModuleManager{
	...
	/**
	 * The annotation that must go on the classes that are a module, it tells
	 * the module manager to look deeper into them for the ModInit, Parser, and
	 * Help methods
	 *
	 * @author Coolway99
	 */
	...
	public static @interface Module{
		/**
		 * What type the internal help method is, can be of type component,
		 * html, or txt
		 *
		 * @see HelpType
		...
		/**
		 * The minimum version of JLime this module is compatible with, will be
		 * mandatory in the future, for now defaults to the current version. If
		 * the current version is lower, the module will fail to load.
		 *
	...
	/**
	 * The annotation that signifies that this is the method that inits the
	 * module, and said method should return the modname (the unique name that
	 * is used to point to that module at runtime). Any module initialization
	 * should take place here, as when this method is found it is ran<br />
	 * <h1>WARNING</h1>This method is ran as soon as it is found, this means
	 * that some class files in the same jar may not have been loaded yet!<br />
	 *
	 * @author Coolway99
	 */
	@Retention(RetentionPolicy.RUNTIME)
	...
	/**
	 * The method that does the blunt work, is called with all the data required
	 * to process commands Should return -1 if it consumed the command call (as
	 * in, "yes I have the requested command and have ran it"), -2 if there is
	 * an error (you must output the error to the consoleOutput though), and -3
	 * if "no, I didn't have anything for that, keep looking".
	 *
	 * @author Coolway99
	 */
	@Retention(RetentionPolicy.RUNTIME)
	...
	/**
	 * Proper documentation is key, not having it is a pain in the butt.
	 * Therefore, help files are mandatory for all files and will be called
	 * through this method. Simply put, it will return a component or a string
	 * depending on what you specified in the Method annotation, and will put it
	 * into the help menu for the user to read.
	 *
	 * @see HelpType
	 * @author Coolway99
	 */
	...
	/**
	 * Initialization, will scan the given directory and call add(File) on any
	 * module found there
	 *
	 * @param directory
	 */
	...
	/**
	 * Attempt to add any module files. If it finds no modules then the jar is
	 * unloaded
	 *
					...
					if(annotation.minProgVersion() > GUI.version){
						JOptionPane
								.showMessageDialog(
										...
										"Warning, module file "
												+ file.getName()
												+ " is for a newer version of JLime"
												+ "\nPlease update JLime or tell the Mod Author about this problem"
												+ "\n Current Version:" + GUI.version
												+ ", Required version:"
												+ annotation.minProgVersion(), "Module Error",
	...
	/**
	 * Is called by CommandParser to check the specified modules to see if they
	 * contain the command
	 *
	 ...
	 * @param consoleOutput The array to-be-outputted to the console, any
	...
	/**
	 * The list of files that are actually loaded since files can have more than
	 * one module
	 *
